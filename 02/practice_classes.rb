...
class Candidate
  attr_reader :name, :email, :interviewers
  ...
  def self.send_email(&block)
    puts "Sending E-mail to: #{@email}"
    yield
  end

  private
    def choose_interviewer
      2.times do
        num = rand(5)
        @interviewers.push(INTERVIEWERS[num][:name])
      end
    end
end
...
CANDIDATES.each do |candidate_data|
  array_of_candidates.push(Candidate.new(candidate_data[:name], candidate_data[:email]))
end

array_of_candidates.each do |candidate|
  Candidate.send_email do
    puts "Hello candidate: #{candidate.name}"
    puts "Your interviewers are: #{candidate.interviewers[0]} y #{candidate.interviewers[1]}"
    puts
  end
end
